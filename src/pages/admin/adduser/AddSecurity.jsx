    ...
    if (Object.keys(tempErrors).length === 0) {
      ...
      try {
        const res = await fetch("http://localhost:5000/api/staff/register", {
          ...
          body: JSON.stringify({ ...security, role: "security", registeredDate: new Date().toLocaleString() }),
        });
        ...
        if (res.ok && data.success) {
          if (data.message.includes("email sent") || !data.message.includes("failed to send")) {
            ...
            toast.warning("Security registered, but failed to send confirmation email.");
          }
          navigate("/admin/userdetails/security");
        } else {
          throw new Error(data.message || "Failed to add security");
        }
      } catch (err) {
